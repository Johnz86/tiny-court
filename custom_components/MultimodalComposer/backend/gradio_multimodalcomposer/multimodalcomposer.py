"""MultimodalComposer — Tiny Court's chat-composer component.

A multimodal text + file input (templated from ``gr.MultimodalTextbox``) restyled
as the courtroom's smoked-glass composer bar: a type-aware attachment tray (image
thumbnails, audio/video/file cards) and an inline microphone capture overlay.
"""
...
class MultimodalComposer(FormComponent):
    """
    A chat composer: a textarea for entering a message together with multimedia
    file uploads and optional microphone capture. Its value is a dict of the form
    ``{"text": str, "files": list[str]}``. Restyled for Tiny Court of Everyday
    Crimes as the courtroom's smoked-glass composer bar.
    """
    ...
    def postprocess(self, value: MultimodalValue | str | None) -> MultimodalData | None:
        ...
        if not isinstance(value, (dict, str)):
            raise ValueError(
                f"MultimodalComposer expects a string or a dictionary with optional keys 'text' and 'files'. Received {value.__class__.__name__}"
            )
